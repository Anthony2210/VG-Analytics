...
### Tableaux synthèse
```{r recuperation donnée, echo=FALSE, warning=FALSE, message=FALSE}
library(dplyr)
library(DBI)
library(RMySQL)
library(knitr)
...
get_column_names <- function(table_name, con) {
  column_info <- dbGetQuery(con, paste("DESCRIBE", table_name))
  return(column_info$Field)
}
# Nom de la table pour laquelle 
# vous voulez obtenir des informations sur les colonnes
table_name <- "jeu"
table_name2 <- "statistique"

# Récupérer et afficher les informations sur 
# les colonnes de la table spécifiée
column_names_jeu <- get_column_names("jeu", con)
column_names_statistique <- get_column_names("statistique", con)

# Creation d'un dataframe
steam_game_df <- data.frame(
  `Nom colonne` = column_names_jeu[1:9],
  Type = c("Integer", "String", "Date", "Float", "Integer", "Integer",
           "Integer", "Integer", "String"),
  Signification = c("Identifiant du jeu sur Steam", "Nom du jeu",
                    "Date de sortie du jeu", "Prix initial du jeu",
                    "Evaluation moyenne des critiques",
                    "Nombre de votes positifs", 
                    "Nombre de votes négatifs",
                    "Temps de jeu moyen", "Développeurs du jeu"),
  Caractéristique = c("Clé primaire, unique", "Pas de valeurs manquantes", 
                      "Pas de valeurs manquantes", 
                      "Pas de valeurs manquantes", "Peut être 0",
                      "Peut être 0", "Peut être 0", "Temps en minutes",
                      "-")
)

# Création des tableaux avec kable et kableExtra
kable(steam_game_df, "latex", longtable = TRUE, booktabs = TRUE, 
      caption = "Player Counts on Steam (950 x 9)") %>%
...
```{r steam-charts-table, echo=FALSE, warning=FALSE}
# Creation d'un dataframe
steam_charts_df <- data.frame(
  `Nom colonne` = column_names_statistique[1:6],
  Type = c("Integer", "String", "Float", "Float", "Float", "Integer"),
  Signification = c("Identifiant unique du jeu sur Steam", 
                    "Mois où les statistiques sont prises", 
                    "Nombre moyen de joueurs actifs", 
                    "Gain du nombre de joueurs par rapport
                    au mois précédent",
                    "Pourcentage du gain du nombre de joueurs
                    par rapport au mois précédent",
                    "Nombre maximal de joueurs actifs"),
  Caractéristique = c("Clé primaire, unique", "Format de date",
                      "Peut être 0", "-", "-", "Peut être 0")
)

# Création des tableaux avec kable et kableExtra
kable(steam_charts_df, format = "latex", longtable = TRUE, 
      booktabs = TRUE, escape = FALSE, 
      caption = "Steam Charts on Steam (52287 x 5)") %>%
  kable_styling(full_width = FALSE) %>%
...
```{r moyenneinvisible, echo=FALSE, warning=FALSE}
query <- "
...
```{r results-table moyenneinvisible, echo=FALSE, warning=FALSE}
...
```{r catégorie des prix invisible, echo=FALSE, warning=FALSE}
query <- "
SELECT 
  CASE 
...
```{r results-table catégorie prix invisible, echo=FALSE, warning=FALSE}
...
```{r developpeurs invisible, echo=FALSE, warning=FALSE}
query <- "
SELECT 
  j.Developers, 
  ROUND(AVG(j.MetacriticScore), 2) as AvgMetacriticScore, 
  ROUND(AVG(s.Gain), 2) as AvgGain
FROM jeu j
JOIN statistique s ON j.AppID = s.AppID
GROUP BY j.Developers
HAVING AVG(j.MetacriticScore) > 75 AND AVG(s.Gain) > 0
ORDER BY AvgMetacriticScore DESC, AvgGain DESC
LIMIT 10;
"
results_dev <- dbGetQuery(con, query)
```

```{r results-table developpeurs invisible, echo=FALSE, warning=FALSE}
results_dev <- results_dev %>%
  rename(
    "Développeur" = Developers,
    "Moyenne des notes Metacritic" = AvgMetacriticScore,
    "Moyenne des gains de joueurs"= AvgGain
  )
kable(results_dev, "latex", booktabs = TRUE, longtable = TRUE, 
      caption= "Requête 3, Les developpeurs" ) %>%
  kable_styling(font_size = 10) %>%  
...
```{r plus/minus invisible, echo=FALSE, warning=FALSE}
query <- "
...
```{r results-table plus/minus invisible, echo=FALSE, warning=FALSE}
...
```{r gamecount invisible, echo=FALSE, warning=FALSE}
query <- "
WITH ScoredGames AS (
  SELECT
    j.AppID,
...
```{r results-table gamecount invisible, echo=FALSE, warning=FALSE}
...
```{r modélisation de price invisible, echo=FALSE, warning=FALSE, message=FALSE}
library(moments)
library(magrittr)
...
```{r histogramme invisible, echo=FALSE, warning=FALSE, message=FALSE}
library(ggplot2)
...
\newpage
```{r nuage de point invisible, echo=FALSE, warning=FALSE, message=FALSE}
library(ggplot2)
library(dplyr)
...
```{r diagramme invisible,echo=FALSE, warning=FALSE, fig.width=6, fig.height=4, message=FALSE}
library(dplyr)
library(ggplot2)
library(ggpattern)
...
```{r nettoyage des données, message=FALSE}
...
```{r results-table moyenne, warning=FALSE}
results_moy <- results_moy %>%
  rename(
    `Moyenne des prix` = Price,
    `Moyenne des notes Metacritic` = MetacriticScore
...
```{r catégorie des prix, warning=FALSE}
query <- "
SELECT 
  CASE 
    WHEN j.Price = 0 THEN 'Gratuit'
...
```{r results-table catégorie prix, warning=FALSE}
results_pri <- results_pri %>%
  rename(
    "Catégorie de Prix" = PriceCategory,
    "Moyenne des joueurs actifs" = AvgActivePlayers
...
```{r developpeurs, echo=FALSE, warning=FALSE}
query <- "
SELECT 
  j.Developers, 
  ROUND(AVG(j.MetacriticScore), 2) as AvgMetacriticScore, 
...
```{r results-table developpeurs, echo=FALSE, warning=FALSE}
results_dev <- results_dev %>%
  rename(
    "Développeur" = Developers,
    "Moyenne des notes Metacritic" = AvgMetacriticScore,
...
```{r results-table plus/minus, warning=FALSE}
results_plusminus <- results_plusminus %>%
  rename(
    "Jeu" = Name,
    "Prix" = Price,
...
results_gamecount <- dbGetQuery(con, query)
```
```{r results-table gamecount2, warning=FALSE}
results_gamecount <- results_gamecount %>%
  rename(
    "Catégorie du jeu" = Category,
    "Nombre de jeu" = GameCount,
...
```{r modélisation-de-price, warning=FALSE, message=FALSE}
library(moments)
library(magrittr)
...
```{r histogramme, warning=FALSE, message=FALSE}
library(ggplot2)
...
```{r nuage de point, warning=FALSE, message=FALSE}
library(ggplot2)
library(dplyr)
...
```{r diagramme, warning=FALSE, fig.width=6, fig.height=4, message=FALSE}
library(dplyr)
library(ggplot2)
library(ggpattern)
